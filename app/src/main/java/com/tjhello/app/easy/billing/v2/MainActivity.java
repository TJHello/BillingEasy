...
import com.tjhello.app.easy.billing.NextActivity;
...
public class MainActivity extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        BillingEasyStatic.addProductConfig(ProductType.TYPE_INAPP_CONSUMABLE,INAPP_CONSUMABLE_CODE_ARRAY);
        BillingEasyStatic.addProductConfig(ProductType.TYPE_INAPP_NON_CONSUMABLE,INAPP_NON_CONSUMABLE_CODE_ARRAY);
        BillingEasyStatic.addProductConfig(ProductType.TYPE_SUBS,SUBS_CODE_ARRAY);

        //添加监听器放到onCreate里更安全
        BillingEasyStatic.addListener(mBillingEasyListener);

        BillingEasyStatic.init(this.getApplicationContext());//仅需调用一次
    ...
    private class MyBillingEasyListener implements BillingEasyListener {
        ...
        public void onQueryOrder(@NonNull BillingEasyResult result, @NonNull List<PurchaseInfo> purchaseInfoList) {
            //这里可以对消耗失败的商品进行二次确认购买或者重新消耗，以及发放商品
            //处理订单示例
            utilPurchase(result,purchaseInfoList);
